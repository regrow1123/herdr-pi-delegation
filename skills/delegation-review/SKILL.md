...
### 피드백 전달 방법 (스크립트 재호출 — 파일 버스 연장)
각 수정 라운드는 **`herdr-spawn-pi.sh`를 다시 호출**한다 (새 pi 기동, 재사용 금지 원칙 유지). 이전 결과 파일 경로 + 지적사항을 task에 함께 실어 보낸다:

```bash
~/.hermes/bin/herdr-spawn-pi.sh "이전 결과 파일(~/.hermes/delegation-results/<이전파일>.md)을 읽고,
아래 지적사항을 반영해서 수정한 뒤 새 결과 파일을 저장해:
- 지적 1: ...
- 지적 2: ..."
```

- 새 pi가 이전 결과 파일을 **읽고** (read), 지적사항 반영 후 **새 파일로 저장**한다.
- 프롬프트에 "이전 결과 파일 경로"를 명시하는 것이 핵심 — pi는 이전 대화 맥락이 없으므로 파일로 맥락을 넘긴다.
- **같은 pi 세션에 직접 `herdr agent prompt <이름> "..."` 를 쓰는 resume 방식도 가능**하지만, "항상 새 pi" 원칙과 후속 로그 추적(file 버스)을 위해 스크립트 재호출을 기본으로 한다.
